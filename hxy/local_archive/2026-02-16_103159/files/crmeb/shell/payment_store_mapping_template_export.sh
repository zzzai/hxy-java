...
usage() {
  cat <<'EOF'
用法：
  ./shell/payment_store_mapping_template_export.sh [--out /path/store_mapping.csv] [--include-deleted 0|1] [--blank-sub-mchid 0|1]
...
echo "[store-mapping-template] total_store_count=${total_count}, current_mapped_count=${current_mapped_count}, template_blank_count=${template_blank_count}, invalid_sub_mchid_count=${invalid_sub_count}"
