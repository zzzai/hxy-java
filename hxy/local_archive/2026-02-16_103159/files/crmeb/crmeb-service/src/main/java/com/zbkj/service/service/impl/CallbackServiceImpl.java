...
public class CallbackServiceImpl implements CallbackService {
    ...
    private void verifyOrderByQuery(StoreOrder storeOrder, CallbackVo callbackVo) {
        ...
        Map<String, String> queryPayVo = buildOrderQueryPayVo(storeOrder.getOutTradeNo(), payConfig);
        MyRecord queryRecord = wechatNewService.payOrderQuery(queryPayVo);
    ...
    private Map<String, String> buildOrderQueryPayVo(String outTradeNo, WeChatPayChannelConfig payConfig) {
    ...
    public String weChatRefund(String xmlInfo) {
        ...
        if (!OrderRefundStateMachine.canSwitchToSuccess(storeOrder.getRefundStatus())) {
            logger.warn("微信退款订单状态非法，拒绝变更==> refundStatus={}, orderNo={}, rawData==>{}", storeOrder.getRefundStatus(), outRefundNo, xmlInfo);
            return refundRecord.getStr("returnXml");
        }
        ...
        if (update) {
            ...
            if (ObjectUtil.isNotNull(latestOrder) && OrderRefundStateMachine.isFinalSuccess(latestOrder.getRefundStatus())) {
                logger.warn("微信退款订单并发更新已完成==> orderNo={}, rawData==>{}", outRefundNo, xmlInfo);
                return refundRecord.getStr("returnXml");
            }
