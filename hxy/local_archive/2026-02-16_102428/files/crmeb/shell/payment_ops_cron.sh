...
usage() {
  cat <<'USAGE'
用法：
  ./shell/payment_ops_cron.sh [--apply] [--remove] [--webhook URL] [--type wecom|dingtalk|feishu] [--report-notify 0|1] [--report-cron EXPR] [--ticket-notify 0|1] [--ticket-cron EXPR] [--ticket-max-rows N] [--ticket-amount-p1-threshold-cent N] [--ticket-owner-map-file FILE] [--ticket-owner-default NAME] [--ticket-owner-p1 NAME] [--decision-ticket-notify 0|1] [--decision-ticket-cron EXPR] [--warroom-notify 0|1] [--warroom-cron EXPR] [--warroom-window-hours N] [--go-nogo-notify 0|1] [--go-nogo-cron EXPR] [--go-nogo-require-apply-ready 0|1] [--go-nogo-require-booking-repair-pass 0|1] [--morning-bundle-notify 0|1] [--morning-bundle-cron EXPR] [--morning-bundle-require-apply-ready 0|1] [--morning-bundle-require-booking-repair-pass 0|1] [--morning-bundle-window-hours N] [--morning-bundle-window N] [--morning-bundle-tail N] [--morning-bundle-strict-preflight 0|1] [--status-notify 0|1] [--status-cron EXPR] [--status-refresh 0|1] [--status-refresh-require-apply-ready 0|1] [--status-require-booking-repair-pass 0|1] [--status-require-decision-chain-pass 0|1] [--status-max-summary-age-minutes N] [--status-max-recon-age-days N] [--status-max-daily-report-age-days N] [--contract-notify 0|1] [--contract-cron EXPR] [--contract-require-all 0|1] [--sla-notify 0|1] [--sla-cron EXPR] [--sla-lookback-days N] [--sla-days N] [--retention-notify 0|1] [--retention-cron EXPR] [--retention-keep-days N] [--booking-repair-notify 0|1] [--booking-repair-cron EXPR] [--booking-repair-window-hours N] [--booking-repair-apply 0|1] [--decision-chain-notify 0|1] [--decision-chain-cron EXPR] [--cron-health-notify 0|1] [--cron-health-cron EXPR] [--mapping-audit-notify 0|1] [--mapping-audit-cron EXPR] [--mapping-audit-strict-missing 0|1] [--cutover-gate-notify 0|1] [--cutover-gate-cron EXPR] [--cutover-gate-require-apply-ready 0|1] [--cutover-gate-require-booking-repair-pass 0|1] [--cutover-gate-mapping-strict-missing 0|1] [--cutover-gate-require-mapping-green 0|1] [--cutover-gate-require-mock-green 0|1] [--cutover-gate-owner-map-file FILE] [--db-host HOST] [--db-port PORT] [--db-name NAME] [--db-user USER] [--db-pass PASS] [--mysql-defaults-file FILE] [--window N] [--tail N]
...
参数：
  ...
  --cutover-gate-notify N      切换门禁任务开关（0=关闭, 1=开启，默认 0）
  --cutover-gate-cron EXPR     切换门禁 cron 表达式（默认 42 2 * * *）
  --cutover-gate-require-apply-ready 0|1         门禁是否要求 apply-ready（默认 0）
  --cutover-gate-require-booking-repair-pass 0|1 门禁是否要求 booking_verify_repair=PASS（默认 0）
  --cutover-gate-mapping-strict-missing 0|1      门禁映射审计 missing 是否阻断（默认 0）
  --cutover-gate-require-mapping-green 0|1       门禁是否要求 mapping_audit=GREEN（默认 0）
  --cutover-gate-require-mock-green 0|1          门禁是否要求 mock_replay=GREEN（默认 0）
  --cutover-gate-owner-map-file FILE             门禁 mock 回放 owner 规则（默认沿用 ticket owner map）
...
if [[ "${GONOGO_NOTIFY}" == "1" ]]; then
  gonogo_line="${GONOGO_CRON_EXPR} cd ${ROOT_DIR} && mkdir -p ${LOCK_DIR} && { ${ENV_PREFIX} flock -n ${LOCK_DIR}/payment_go_nogo.lock ./shell/payment_go_nogo_decision.sh --require-apply-ready ${GONOGO_REQUIRE_APPLY_READY} --require-booking-repair-pass ${GONOGO_REQUIRE_BOOKING_REPAIR_PASS} || echo \"[lock] go-nogo skipped: lock busy\"; } >> ${RUNTIME_DIR}/payment_go_nogo_cron.log 2>&1"
fi

morning_bundle_line=""
if [[ "${MORNING_BUNDLE_NOTIFY}" == "1" ]]; then
  morning_bundle_cmd="./shell/payment_ops_morning_bundle.sh --window ${MORNING_BUNDLE_WINDOW_MINUTES} --tail ${MORNING_BUNDLE_TAIL_LINES} --window-hours ${MORNING_BUNDLE_WINDOW_HOURS} --require-apply-ready ${MORNING_BUNDLE_REQUIRE_APPLY_READY} --require-booking-repair-pass ${MORNING_BUNDLE_REQUIRE_BOOKING_REPAIR_PASS}"
...
if [[ "${CUTOVER_GATE_NOTIFY}" == "1" ]]; then
  ...
  cutover_gate_cmd="./shell/payment_cutover_gate.sh --require-apply-ready ${CUTOVER_GATE_REQUIRE_APPLY_READY} --require-booking-repair-pass ${CUTOVER_GATE_REQUIRE_BOOKING_REPAIR_PASS} --mapping-strict-missing ${CUTOVER_GATE_MAPPING_STRICT_MISSING} --require-mapping-green ${CUTOVER_GATE_REQUIRE_MAPPING_GREEN} --require-mock-green ${CUTOVER_GATE_REQUIRE_MOCK_GREEN} --owner-default ${TICKET_OWNER_DEFAULT} --owner-p1 ${TICKET_OWNER_P1}"
