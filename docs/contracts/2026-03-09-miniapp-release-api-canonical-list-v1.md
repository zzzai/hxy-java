...
| `/pages/booking/technician-list` | 技师列表查询 | GET | `/booking/technician/list` | `storeId` | `data[]:{id,name,avatar,introduction,tags,rating,serviceCount}`；页面额外使用的 `title/specialties/status` 当前无 backend 绑定 | `-` | 合法空列表 `[]`；无 `degraded` | `Can Develop / Cannot Release` |
| `/pages/booking/technician-detail` | 技师详情查询 | GET | `/booking/technician/get` | `id` | `data:{id,name,avatar,introduction,tags,rating,serviceCount}` | `-` | `success(null)` 只算空结果，不是成功样本 | `Can Develop / Cannot Release` |
| `/pages/booking/technician-detail` | 技师时段查询 | GET | `/booking/slot/list-by-technician` | `technicianId`, `date` | `data[]:{id,technicianId,technicianName,technicianAvatar,slotDate,startTime,endTime,isOffpeak,offpeakPrice,status}` | `-` | 合法空列表 `[]`；无 `degraded` | `Can Develop / Cannot Release` |
| `/pages/booking/order-confirm` | 创建预约订单 | POST | `/booking/order/create` | `timeSlotId`, `spuId`, `skuId?`, `userRemark`, `dispatchMode` | `orderId` | `TIME_SLOT_NOT_AVAILABLE(1030003001)` | fail-close；helper 失败不跳详情；当前 page payload 绑定未闭环 | `Can Develop / Cannot Release` |
| `/pages/booking/order-list` | 预约列表查询 | GET | `/booking/order/list` | 当前 backend 不消费 `pageNo`, `pageSize`, `status` | controller 实际返回 `data[]:{id,orderNo,status,servicePic,serviceName,bookingDate,bookingStartTime,bookingEndTime,duration,payPrice}`；页面当前按 `data.list/data.total` 读取，且 `payOrderId` 无响应绑定 | `-` | `code=0` 也可能空渲染；不得写成成功样本 | `Can Develop / Cannot Release` |
| `/pages/booking/order-detail` | 预约详情查询 | GET | `/booking/order/get` | `id` | `data:{id,orderNo,technicianName,technicianAvatar,serviceName,bookingDate,bookingStartTime,bookingEndTime,duration,originalPrice,discountPrice,payPrice,status,userRemark}`；`payOrderId` 当前无响应绑定 | `BOOKING_ORDER_NOT_OWNER(1030004006)`；miss=`success(null)` | 无 `degraded`；空结果只算 not-found 态 | `Can Develop / Cannot Release` |
| `/pages/booking/order-list` / `/pages/booking/order-detail` | 取消预约 | POST | `/booking/order/cancel` | `id`, `reason` | `true` | `BOOKING_ORDER_NOT_EXISTS(1030004000)`, `BOOKING_ORDER_CANNOT_CANCEL(1030004005)`, `BOOKING_ORDER_NOT_OWNER(1030004006)` | fail-close；成功后仅 `REFRESH_ONCE`；无发布级写后回读证据 | `Can Develop / Cannot Release` |
...
| `/pages/booking/addon` | 加钟 / 升级 / 加项目 | POST | `/app-api/booking/addon/create` | `parentOrderId`, `addonType`, `spuId?`, `skuId?` | `orderId` | `TIME_SLOT_NOT_AVAILABLE(1030003001)`, `BOOKING_ORDER_NOT_EXISTS(1030004000)`, `BOOKING_ORDER_STATUS_ERROR(1030004001)`, `BOOKING_ORDER_NOT_OWNER(1030004006)` | fail-close；页面当前只提交 `parentOrderId,addonType`，存在 `pseudo success / no-op risk` | `Can Develop / Cannot Release` |
