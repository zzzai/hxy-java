...
| canonical truth | 当前 FE wrapper / helper | 当前 controller | runtime binding | 当前 scope |
|---|---|---|---|---|
| `GET /booking/technician/list` | `BookingApi.getTechnicianList` / `loadTechnicianList` | `AppTechnicianController#getTechnicianList` | `Aligned` | `Query-Only Active / Can Release=Yes` |
| `GET /booking/slot/list-by-technician` | `BookingApi.getTimeSlots` / `loadTimeSlots` | `AppTimeSlotController#getTimeSlotsByTechnician` | `Aligned` | `Query-Only Active / Can Release=Yes` |
...
| `GET /booking/order/get` | `BookingApi.getOrderDetail` / `goToOrderDetail` | `AppBookingOrderController#getOrder` | `Aligned` | `Query-Only Active / Can Release=Yes` |
| `GET /booking/order/list` | `BookingApi.getOrderList` | `AppBookingOrderController#getOrderList` | `Aligned` | `Query-Only Active / Can Release=Yes` |
...
说明：
- `loadTechnicianDetail -> BookingApi.getTechnician -> GET /booking/technician/get` 仍是当前 helper 的真实引用关系，并且当前属于 query-only `ACTIVE`。
...
| canonical truth | 当前稳定 errorCode 证据 | failureMode | retryClass | 当前真实恢复口径 |
|---|---|---|
| `GET /booking/technician/list` | `-`；controller/service 不做技师存在性校验，空列表 `[]` 合法 | `FAIL_OPEN` | `NO_AUTO_RETRY` | 空态合法；无服务端 `degraded` |
| `GET /booking/technician/get` | `-`；miss 当前返回 `success(null)` | `FAIL_OPEN` | `NO_AUTO_RETRY` | 当前不得按 `1030001000/1030001001` 分支 |
| `GET /booking/slot/list-by-technician` | `-`；当前 path 无 `technicianId` 有效性错误码守卫，空列表 `[]` 合法 | `FAIL_OPEN` | `NO_AUTO_RETRY` | 空态合法；无服务端 `degraded` |
...
| `GET /booking/order/get` | `BOOKING_ORDER_NOT_OWNER(1030004006)`；订单不存在当前返回 `success(null)`，不是 `BOOKING_ORDER_NOT_EXISTS(1030004000)` | `FAIL_CLOSE` | `NO_AUTO_RETRY` | 越权阻断；miss 按 `null` 处理 |
| `GET /booking/order/list` | `-`；空列表 `[]` 合法 | `FAIL_OPEN` | `REFRESH_ONCE` | 页面支持手动刷新；无服务端 `degraded` |
| `POST /booking/order/cancel` | `BOOKING_ORDER_NOT_EXISTS(1030004000)`、`BOOKING_ORDER_NOT_OWNER(1030004006)`、`BOOKING_ORDER_CANNOT_CANCEL(1030004005)` | `FAIL_CLOSE` | `REFRESH_ONCE` | 失败不刷新；成功后刷新一次 |
...
说明：
- 当前 booking runtime 没有稳定 `MANUAL_RETRY_3` contract path。
- create / add-on 的“返回上游重选后重提”属于产品恢复动作，不是当前 helper 暴露的重试类。
...
1. `docs/products/miniapp/2026-03-15-miniapp-booking-runtime-release-evidence-review-v1.md` 已把 booking 域固定为 `Doc Closed / Can Develop / Cannot Release`。
2. `docs/products/miniapp/2026-03-12-miniapp-business-function-truth-ledger-v1.md` 仍将 BF-022/BF-023/BF-024 保持为 `PLANNED_RESERVED`。
3. 当前自动化证据仅覆盖 wrapper/helper 对齐与成功/失败 smoke，不等于真实 release sample 已归档。
4. 因此本文件当前只固定 contract truth，并把 query-only 与 write-chain 的 release scope 分开写清，不把写链误升为 `Can Release=Yes`。
