...
| 门店 -> 店长后台账号映射 | 当前未核出稳定 `storeId -> managerAdminUserId` 关系、字段或关系表 | `未核出` |
| 后台执行动作账号 | `managerClaimedByUserId / managerLatestActionByUserId` 只表示执行认领/处理/闭环的后台操作人 | `已核实` |
| 自动通知目标账号 | 当前没有稳定账号级接收人真值 | `未核出` |
| 现成 outbox 模式 | booking 模块内已有 `TechnicianCommissionSettlementNotifyOutboxDO` 作为 outbox 参考模式 | `已核实` |
...
4. “差评通知已经具备账号级权限路由和组织归属真值。”
5. “booking review 当前已经存在独立 notify outbox 数据模型。”

## 6. 当前可成立的说法
1. 差评当前已具备 admin-only 店长待办治理层。
2. 差评提交成功后的同步触发点已经存在，可以写“通知意图”，但不能直接诚实写成“账号已收到通知”。
3. booking 模块内已有其他业务域使用 outbox + 异步派发的模式，可作为实现参考。
4. 如果后续要做账号级通知，必须先补稳定的 `storeId -> managerAdminUserId` 真值模型。
...
- 在 `managerAdminUserId` 真值正式闭环之前，不得把本专题写成“自动通知已上线”。
