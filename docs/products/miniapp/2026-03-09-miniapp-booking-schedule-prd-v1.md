...
- 本文约束：
  ...
  - 当前写链虽然已完成商品来源真值收口，但真实发布样本、gray / rollback / sign-off 仍未闭环，不得回退成旧稳定口径。
...
| 技师详情 | `/pages/booking/technician-detail` | `BookingApi.getTechnician(id)` + `BookingApi.getTimeSlots(technicianId,date)`；选择日期、选择时段后进入服务选择页 | route:`id`,`storeId`；detail backend 稳定字段:`avatar`,`name`,`introduction`,`tags`,`rating`,`serviceCount`；页面 fallback:`title`；slot backend 稳定字段:`id`,`technicianId`,`technicianName`,`technicianAvatar`,`slotDate`,`startTime`,`endTime`,`isOffpeak`,`offpeakPrice`,`status`；本地日期:`date`,`label`,`weekDay` | `Query-Only Active`；下游下单入口 `Can Develop / Cannot Release` |
| 服务选择 | `/pages/booking/service-select` | `SpuApi.getSpuPage` 拉服务列表，`SpuApi.getSpuDetail` + SKU 选择后把显式 `spuId/skuId` 回传给下游写链页面 | route:`flow`,`timeSlotId?`,`technicianId?`,`storeId?`,`parentOrderId?`,`addonType?`；列表字段:`id`,`name`,`introduction`,`picUrl`,`price`；详情字段:`id`,`name`,`skus[]` | `Can Develop / Cannot Release` |
| 确认预约 | `/pages/booking/order-confirm` | 读取技师、时段、服务商品后提交 `BookingApi.createOrder` | route:`timeSlotId`,`technicianId`,`storeId`,`spuId`,`skuId`；页面稳定可见:`slotDate`,`startTime`,`endTime`,`duration`,`isOffpeak`,`offpeakPrice`,`avatar`,`name`,`serviceName`；页面 fallback:`title`；提交:`timeSlotId`,`spuId`,`skuId`,`userRemark`,`dispatchMode=1(helper追加)` | `Can Develop / Cannot Release` |
...
| 加钟服务 | `/pages/booking/addon` | 先用 `BookingApi.getOrderDetail(parentOrderId)` 读取母单；`addonType=1` 可沿用母单商品；`addonType=2/3` 先跳服务选择页，再提交 `BookingApi.createAddonOrder` | route:`parentOrderId`,`addonType?`,`spuId?`,`skuId?`；展示:`serviceName`,`bookingStartTime`,`bookingEndTime`,`technicianName`；提交:`parentOrderId`,`addonType`,`spuId`,`skuId`；页面 `remark` 仅本地保留，当前不入请求 | `Can Develop / Cannot Release` |
...
|---|---|---|---|---|
| 创建预约 | `/pages/booking/service-select`; `/pages/booking/order-confirm` | `technician-detail -> service-select -> order-confirm` 已固定；`buildBookingCreatePayload` 只接受显式 `spuId/skuId`；`submitBookingOrderAndGo` 只在 `code === 0` 时跳 `/pages/booking/order-detail` | `Can Develop`；`Cannot Release` | 停留确认页，结束 loading；用户返回服务选择页改选项目/规格后重提 |
...
| 加钟 / 升级 / 加项目 | `/pages/booking/service-select`; `/pages/booking/addon` | `buildBookingAddonPayload` 已固定 `addonType=1` 可沿用母单商品、`addonType=2/3` 必须显式 `spuId/skuId`；`submitAddonOrderAndGo` 只在 `code === 0` 时跳 `/pages/booking/order-detail` | `Can Develop`；`Cannot Release` | 停留加钟页，不自动跳转；用户重新选择 `addonType` 或返回服务选择页补选项目 |
...
3. 只承认当前真实请求字段：
   ...
   - cancel：`id`,`reason`
   - addon：`parentOrderId`,`addonType`,`spuId`,`skuId`
4. 当前 booking 页面不消费 `degraded` / `degradeReason` 字段；恢复动作只按当前 helper 行为与页面结构态定义。
5. `order-confirm` 当前通过 `GET /booking/slot/get?id=` 读取时段详情，服务商品来源只认 `service-select` 回传的显式 `spuId/skuId`；`order-list` 当前透传 `pageNo/pageSize/status`；这几项只算当前页面/helper 真值，不自动等于 formal contract 已闭环。
