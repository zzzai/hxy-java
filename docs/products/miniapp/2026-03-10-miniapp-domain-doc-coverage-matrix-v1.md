...
| Booking Review & Service Recovery | Ready | review-list 历史 / 汇总为 query-side `ACTIVE`；review-add 提交、结果页与后台恢复台账 / 详情 / 看板已实现，但 release proof 仍未闭环 | Full | Full | Full | Full | Full | Full | 93 | 不得复用商品评论真值；缺 feature flag / rollout / runtime sample pack；`picUrls` 仅完成提交链路、历史 / 详情回显未单独闭环；`serviceOrderId` 为 best-effort 回填且仍可为空；自动奖励 / 补偿 / 店长通知未落地 | P0 |
