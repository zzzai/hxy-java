...
| 地址管理 | 地址增删改查 | `/pages/address/list` | `GET/POST/PUT/DELETE /member/address/*` | Member address state | `MEMBER_ADDRESS_*` | 无默认地址返回空并引导新增 | Member Domain Owner | P0 | RB1-P0 | CRUD + 默认地址切换可回归 |
...
| 首页运营位 | 活动卡与店铺上下文一致性 | `/pages/home/index` | `/promotion/*`, `/product/*` | Activity truth binding + catalog version | `TBD_HOME_ACTIVITY_ERROR_CODE` | 动效降级（low-end/reduced-motion），不影响主CTA | Product Owner + Promotion Domain Owner | P1 | RB2-P1 | CTA唯一主路径，活动真值一致 |
| 项目页同源刷新 | 类目切换价库存可约同步 | `/pages/service/catalog` | Product catalog APIs (same snapshot/version) | Catalog snapshot/version truth | `TBD_CATALOG_VERSION_MISMATCH` | 版本不一致时强制全量刷新并提示 | Product Domain Owner | P1 | RB2-P1 | 切类目后无陈旧价格/库存 |
| 加购冲突处理 | 数量冲突与库存不足反馈 | `/pages/service/catalog` | Add-to-cart / booking intent APIs | Booking/Product state | `TBD_ADD_CONFLICT_CODE` | 冲突时回显服务端数量，不自动成功 | Booking + Product Domain Owner | P1 | RB2-P1 | 冲突、库存不足、重试路径清晰 |
| 预约排期 | 时段选择与冲突提醒 | `/pages/booking/schedule` | Booking schedule APIs | Booking schedule truth | `TBD_BOOKING_SLOT_CONFLICT` | 冲突 fail-fast，给可选时段 | Booking Domain Owner | P1 | RB2-P1 | 可约时段、冲突、改约链路完整 |
| 会员资产总账 | 券/积分/权益一致账本 | `/pages/profile/assets` | Coupon/points/member asset APIs | Unified asset ledger truth | `TBD_ASSET_LEDGER_MISMATCH` | 下游失败不吞数据，显示局部降级标签 | Member + Promotion Domain Owner | P1 | RB2-P1 | 资产变动可追溯、口径一致 |
| 搜索发现 | 全局搜索与召回排序 | `/pages/search/index` | Search APIs | Search result truth | `TBD_SEARCH_QUERY_INVALID` | 检索失败保留query并可重试 | Product Domain Owner | P1 | RB2-P1 | 热词/结果/空态/纠错路径可用 |
| 礼品卡域 | 购买/赠送/领取/核销/退款 | `/pages/gift-card/*` | Gift-card domain APIs | Gift-card lifecycle truth | `TBD_GIFT_CARD_*` | 核销或退款失败返回码+补救动作 | Trade + Member Domain Owner | P2 | RB3-P2 | 全生命周期+幂等回归 |
| 邀请有礼 | 关系绑定与奖励到账 | `/pages/referral/*` | Referral domain APIs | Referral reward ledger truth | `TBD_REFERRAL_*` | 到账延迟显示处理中，不伪成功 | Promotion Domain Owner | P2 | RB3-P2 | 关系绑定/奖励发放/冲正可追溯 |
| 技师动态广场 | feed流与互动 | `/pages/technician/feed` | Feed/post/comment APIs | Feed moderation truth | `TBD_FEED_AUDIT_BLOCK` | 审核阻断不影响浏览主链路 | Booking + Content Ops Owner | P2 | RB3-P2 | 发布/审核/展示/降级路径可验收 |
...
- 对于 `TBD_*` 错误码项：进入对应批次前必须先完成文档冻结与门禁锚点补齐。

## 5. Window Ownership and Delivery Dependency
- Window B (UI/PRD): home growth, booking schedule, member asset ledger, search discovery.
- Window C (contracts): addbook conflict, gift-card, referral, technician feed.
- Window D (data/compliance): event taxonomy v2, fulfillment logistics, content compliance styleguide.
- Window A (integration): feature matrix freeze, index governance, cross-window consistency check.
