...
- `booking runtime gate PASS`、focused booking tests PASS、shared local CI booking gate PASS 都只表示“边界被守住”，不表示 `Release Ready`。
- `REQUIRE_BOOKING_MINIAPP_RUNTIME_GATE=0` 只能改变 CI 呈现，不改变 booking 当前 `Can Release=No / No-Go`。
...
| `booking` | `BK-03` | 空态 / gate PASS 归类 | query-only 返回体 + gate / local CI summary | `[] / 0 / null` 只记合法空态；gate PASS 只记边界守住样本，不记 release-ready 样本 | 把空态算成成功样本；把 PASS 算成可放量证据 | 回退样本统计，维持 `No-Go` | Booking Domain Owner + D 窗口 |
| `booking` | `BK-04` | gate 语义与 release decision | `docs/plans/2026-03-16-miniapp-booking-runtime-release-gate-audit-v1.md` + `docs/products/miniapp/2026-03-16-miniapp-booking-runtime-gate-acceptance-sop-v1.md` + shared CI summary | D 侧材料明确写成 `Can Develop=Yes / Can Release=No / Release Ready=No / No-Go`，且不把 `REQUIRE_*` 的 WARN 语义外推成放量许可 | 把 PASS / WARN 写成准发布；把 03-10 contract 中旧 path blocker 快照当成当前 release-ready 证据 | 立即回退 release 口径并重发 acceptance/handoff | Booking Domain Owner + D 窗口 + 发布负责人 |
...
| `reserved-expansion` | `RV-02` | mis-release 守卫 | `RESERVED_DISABLED` 计数 + switch 快照 | 关闭态误返回计数 `=0` | 任一误返回被当作 warning，而不是 mis-release | 立即关闭开关、清空灰度名单，并标记当前批次 `No-Go` | SRE + 发布负责人 |
...
- 当前分支里 `docs/contracts/2026-03-10-miniapp-booking-user-api-alignment-v1.md` 只保留早期 blocker 快照价值；booking 当前 method/path 真值仍以代码、测试和 gate 为准。
- booking 当前统一状态必须继续写成：`Doc Closed / Can Develop / Can Release=No / No-Go`。
...
4. `Reserved runtime` 未实现不得因为 alert / runbook / checklist 完整而放量；任一 `RESERVED_DISABLED` 关闭态命中直接按 mis-release / `No-Go`。
