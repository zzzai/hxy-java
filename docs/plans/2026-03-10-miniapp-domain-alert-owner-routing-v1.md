...
- `RESERVED_DISABLED` 在关闭态或越权范围返回，统一认定为 `P0` 级误发布；不得降级成 warning。
- `BO-004` 的 `code=0` 但 no-op、`Booking` 旧 path 命中、`Member` 分母污染都按发布级事故处理，不降级成普通 warning。
- `booking runtime gate PASS`、focused tests PASS、shared local CI booking gate PASS 只表示边界被守住，不表示 `Release Ready`。
...
|---|---|---|---|---|---|---|---|
| `booking` | `P0` | `GET /booking/technician/list-by-store`、`GET /booking/time-slot/list`、`PUT /booking/order/cancel`、`POST /booking/addon/create` 仍进入发布样本或 allowlist；或任何人把 booking 写成 `Release Ready` / `Can Release=Yes` | Booking Domain Owner + 发布负责人 | 发布值班 -> Booking Domain Owner -> C 窗口 -> 发布负责人 | `5m / 15m / 4h` | 立即冻结 Booking 放量，只保留 query-only 验证范围 | `runId=发布/巡检批次`；`orderId/payRefundId/sourceBizNo=0`；`errorCode=0` |
| `booking` | `P1` | create/cancel/addon 样本不完整、只验单点 `create`、按 `message` 分支、复用 03-10 contract 旧 blocker 快照覆盖当前代码真值、或把 runtime gate / local CI PASS 写成可放量证据 | Booking Domain Owner | Booking Domain Owner -> 发布负责人 -> D 窗口 | `15m / 1h / 24h` | 回退到 canonical 样本集，暂停继续验收，并按 SOP 重分池 | 同上 |
| `booking` | `P2` | query-only 链路稳定，但 create/cancel/addon 仍待补样本；`[] / 0 / null` 空态归类争议 | Booking on-call | Booking on-call -> Booking Domain Owner | `30m / 4h / 72h` | 仅继续补样本，不扩大放量；空态不得计入成功样本 | 同上 |
...
| `reserved-expansion` | `P0` | `RESERVED_DISABLED` 关闭态或越权范围误返回，或 gray runbook 被误当 runtime 成功 | 对应域 on-call + SRE + 发布负责人 | SRE -> 对应域 on-call -> 发布负责人 | `5m / 15m / 4h` | 立即按 mis-release 回滚：关闭开关、清空灰度范围、撤销发布结论 | `runId=灰度/巡检批次`；`orderId/payRefundId/sourceBizNo=有值透传，无值填 0`；`errorCode=保留码真实值` |
...
| `booking` | `docs/plans/2026-03-15-miniapp-booking-runtime-release-runbook-v1.md` 第 `4/5/6/8/9/10` 节；`docs/plans/2026-03-16-miniapp-booking-runtime-release-gate-audit-v1.md` 第 `2/3/4/6/7/8` 节；`docs/products/miniapp/2026-03-16-miniapp-booking-runtime-gate-acceptance-sop-v1.md` 第 `2/3/4/5/6/7/8` 节 |
