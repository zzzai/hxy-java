...
若 `check_booking_refund_replay_ticket_sync_gate.sh` 返回 `BLOCK`，优先检查以下锚点：
- `/booking/refund-notify-log/replay-run-log/detail/page`
- `/booking/refund-notify-log/replay-run-log/sync-tickets`（请求字段 `dryRun/forceResync`）
- summary 新字段：`ticketSyncSuccessCount/ticketSyncSkipCount/ticketSyncFailCount`
...
如需临时降级 replay-ticket-sync 门禁（不建议长期使用）：
...
workflow summary 会输出 `clean_before_tests`、`booking_refund_replay_v2_gate_rc`、`booking_refund_replay_runlog_gate_rc`、`booking_refund_replay_run_summary_gate_rc` 与 `booking_refund_replay_ticket_sync_gate_rc`，用于排查 clean 口径和 replay 门禁状态。
