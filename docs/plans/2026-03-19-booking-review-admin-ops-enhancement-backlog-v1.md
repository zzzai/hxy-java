...
| 编号 | 候选项 | 当前依据 | 价值 | 当前状态 |
|---|---|---|---|---|
| A1 | 台账补门店名 / 技师名 / 会员昵称只读展示 | 当前列表只有 ID，运营阅读成本高 | 提升值班效率 | 可进入设计与真值核对 |
| A2 | 看板卡片支持一键带条件跳回台账 | 当前 `dashboard -> ledger` 只支持返回，不带 drill-down | 缩短排查链路 | 可进入设计 |
| A3 | 详情页增加“最近动作时间线”只读块 | 当前只有最近一次处理备注，没有时间线 | 提升复盘可读性 | 可进入设计 |
| A4 | 台账增加“历史未初始化差评”筛选提示 | 当前 read-path 不自动补齐，容易误读 | 降低误判 | 可进入设计 |
| A5 | 后台历史修复工具评估 | 当前历史差评只在写路径 lazy-init | 为后续数据治理准备 | 仅可做方案，不可先承诺上线 |

## 5. Not Now（当前不得排进本批开发）

| 编号 | 项目 | 原因 |
|---|---|---|
| N1 | 自动通知店长 / 客服 / 区域负责人 | 当前没有稳定账号归属与消息通道真值 |
| N2 | 自动差评补偿 | 当前业务明确未落地 |
| N3 | 自动好评奖励 | 当前业务明确未落地 |
