...
| A14 | 门店绑定治理工作台 | 当前 manager routing 页已新增治理优先级、治理分组、核验状态、来源闭环、治理归口、交接摘要，以及 `只看立即治理 / 来源待闭环 / 长期未核验 / 可观察就绪` 快捷筛选 | 让运营把“缺绑定”进一步拆成可转交、可排队、可复核的治理队列 | 已落地（admin-only, governance-workbench） |
| A15 | notify outbox 跨通道派发审计二期 | 当前 notify outbox 台账已新增 `跨通道审计概览` 卡片、`跨通道结论 / 跨通道说明` 列，以及 review 级 `summary` 聚合接口 | 让运营直接看清“同一条差评两条通道整体是什么状态、是否分裂、是否人工重试后待复核” | 已落地（admin-only, cross-channel-audit-v2） |
| A16 | 门店绑定来源真值闭环 | 当前 manager routing 页已新增 `来源闭环概览`、`来源结论 / 来源说明 / 下一步动作`、`只看来源已确认 / 来源缺失 / 联系人待转绑定 / 联系人缺失 / 来源待复核`，后端统一派生 `sourceTruth*` 字段与 summary | 让运营明确知道当前缺的是“来源登记、联系人主数据还是复核动作”，不再把 `BLOCKED_NO_OWNER` 混成同一类问题 | 已落地（admin-only, source-truth-closure） |
...
2. 下一阶段应转入“运营效率 + 告警联动、发布证据包”两条线，减少重复建设只读真值面。
