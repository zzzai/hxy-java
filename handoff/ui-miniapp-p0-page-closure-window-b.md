...
- 交付形态：`ui/stitch_/closure_p0` 可运行 HTML 原型（按当前分支交付边界，不改后端、不改 overlay 既有页面）
- 覆盖页面：
  - 支付结果页（成功/处理中/失败）
  - 订单列表页（按状态筛选）
  - 售后申请页
...
- 展示策略：
  ...
  - `service + code in [404,405,501,40401,50100]`：识别“旧后端未支持”，明确提示已降级且主流程可继续
- 降级行为：
...
- `code = 0`：正常路径
- `401`：登录态失效（提示重新登录）
- `404/405/501/40401/50100` 或错误信息包含 `未实现/不支持/Not Found`：判定“旧后端不支持”，触发降级
- `5xx`：服务降级（提示并引导异常页）
...
- 页面统一通过 `app.js` 注入异常页上下文参数：`type/code/msg/from`，便于联调追踪。
- 若命中“旧后端不支持”特征，返回结构：
  - `ok: true`
  - `degraded: true`
