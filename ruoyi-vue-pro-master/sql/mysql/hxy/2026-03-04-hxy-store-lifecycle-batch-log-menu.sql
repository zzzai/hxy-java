...
SELECT role_seed.role_id, @store_lifecycle_batch_log_menu_id, '1', '1', b'0', @tenant_id
FROM (
         SELECT @admin_role_id AS role_id
         UNION ALL
         SELECT @operator_role_id
     ) role_seed
WHERE @store_lifecycle_batch_log_menu_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM system_role_menu srm
    WHERE srm.role_id = role_seed.role_id
      AND srm.menu_id = @store_lifecycle_batch_log_menu_id
