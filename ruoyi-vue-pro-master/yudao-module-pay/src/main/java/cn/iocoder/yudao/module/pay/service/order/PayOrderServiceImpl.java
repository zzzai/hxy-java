    ...
    private boolean expireOrder(PayOrderDO order) {
        try {
            ...
            for (PayOrderExtensionDO orderExtension : orderExtensions) {
                ...
                // 关键：微信渠道需先调用关单 API，再落本地关闭，避免渠道侧仍可支付
                if (PayChannelEnum.isWeixin(orderExtension.getChannelCode()) && !payClient.closeOrder(orderExtension.getNo())) {
                    log.error("[expireOrder][extension({}) 渠道关单失败]", orderExtension.getId());
                    return false;
                }
                // 兜底逻辑：将支付拓展单更新为已关闭
