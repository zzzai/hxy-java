...
public abstract class AbstractWxPayClient extends AbstractPayClient<WxPayClientConfig> {
    ...
    protected void doInit(String tradeType) {
        ...
        if (isPartnerModeV3()) {
            // Partner 模式下，mchId 对应 sp_mchid；subMchId/subAppId 补齐到 SDK 配置
     ...
     * @return 下单请求
     */
    protected WxPayPartnerUnifiedOrderV3Request buildPayPartnerUnifiedOrderRequestV3(PayOrderUnifiedReqDTO reqDTO) {
        ...
        if (hasText(config.getSubAppId())) {
            request.setSubAppid(config.getSubAppId());
        }
        request.setOutTradeNo(reqDTO.getOutTradeNo());
        request.setDescription(reqDTO.getSubject());
        request.setAmount(new WxPayPartnerUnifiedOrderV3Request.Amount().setTotal(reqDTO.getPrice())); // 单位分
        request.setTimeExpire(formatDateV3(reqDTO.getExpireTime()));
    ...
    private PayOrderRespDTO doParseOrderNotifyV3(String body, Map<String, String> headers) throws WxPayException {
        ...
        if (isPartnerModeV3()) {
            WxPayPartnerNotifyV3Result response = client.parsePartnerOrderNotifyV3Result(body, signatureHeader);
            WxPayPartnerNotifyV3Result.DecryptNotifyResult result = response.getResult();
            // 2. 构建结果
            Integer status = parseStatus(result.getTradeState());
            String openid = result.getPayer() == null ? null
                    : firstNonBlank(result.getPayer().getSubOpenid(), result.getPayer().getSpOpenid());
    ...
    private PayOrderRespDTO doGetOrderV3(String outTradeNo) throws WxPayException {
        if (isPartnerModeV3()) {
            ...
            WxPayPartnerOrderQueryV3Request request = new WxPayPartnerOrderQueryV3Request()
                    .setSpMchId(config.getMchId())
                    .setSubMchId(config.getSubMchId())
                    .setOutTradeNo(outTradeNo);
            // 执行请求
            WxPayPartnerOrderQueryV3Result response = client.queryPartnerOrderV3(request);

            // 转换结果
            Integer status = parseStatus(response.getTradeState());
            String openid = response.getPayer() == null ? null
                    : firstNonBlank(response.getPayer().getSubOpenid(), response.getPayer().getSpOpenid());
    ...
    public boolean closeOrder(String outTradeNo) {
        try {
            switch (config.getApiVersion()) {
                case API_VERSION_V2:
                    return doCloseOrderV2(outTradeNo);
                case API_VERSION_V3:
                    return doCloseOrderV3(outTradeNo);
                default:
                    throw new IllegalArgumentException(String.format("未知的 API 版本(%s)", config.getApiVersion()));
            }
        } catch (WxPayException e) {
            // 幂等容错：已关单/订单不存在按成功处理，避免任务反复告警
            if (isCloseOrderIdempotentError(e)) {
                return true;
            }
            log.warn("[closeOrder][outTradeNo({}) 关单异常，errCode({}) errMsg({})]", outTradeNo, e.getErrCode(), e.getErrCodeDes(), e);
            return false;
        } catch (Throwable e) {
            log.warn("[closeOrder][outTradeNo({}) 关单异常]", outTradeNo, e);
            return false;
        }
    }

    @Override
    public int getNotifySuccessHttpStatus() {
        // V3 推荐 200/204 且无需返回应答报文；V2 继续沿用 success 文本应答
        return Objects.equals(config.getApiVersion(), API_VERSION_V3) ? 204 : 200;
    }

    @Override
    public String getNotifySuccessBody() {
        return Objects.equals(config.getApiVersion(), API_VERSION_V3) ? null : "success";
    }

    private boolean doCloseOrderV2(String outTradeNo) throws WxPayException {
        WxPayOrderCloseRequest request = WxPayOrderCloseRequest.newBuilder().outTradeNo(outTradeNo).build();
        WxPayOrderCloseResult response = client.closeOrder(request);
        return Objects.equals("SUCCESS", response.getResultCode()) || Objects.equals("SUCCESS", response.getReturnCode());
    }

    private boolean doCloseOrderV3(String outTradeNo) throws WxPayException {
        if (isPartnerModeV3()) {
            WxPayPartnerOrderCloseV3Request request = new WxPayPartnerOrderCloseV3Request()
                    .setSpMchId(config.getMchId())
                    .setSubMchId(config.getSubMchId())
                    .setOutTradeNo(outTradeNo);
            client.closePartnerOrderV3(request);
        } else {
    ...
    private static boolean isCloseOrderIdempotentError(WxPayException e) {
        return ObjectUtils.equalsAny(e.getErrCode(),
                "ORDERNOTEXIST", "ORDER_NOT_EXIST", "ORDER_CLOSED", "ORDER_ALREADY_CLOSED");
    }
    ...
    private PayRefundRespDTO doUnifiedRefundV3(PayRefundUnifiedReqDTO reqDTO) throws Throwable {
        ...
        if (isPartnerModeV3()) {
            ...
            WxPayRefundV3Request request = new WxPayRefundV3Request()
                    ...
                    .setNotifyUrl(reqDTO.getNotifyUrl());
            // 2.1 执行请求
            response = client.refundV3(request);
        }
        // 2.2 创建返回结果
    ...
    private PayRefundRespDTO parseRefundNotifyV3(String body, Map<String, String> headers) throws WxPayException {
        ...
        SignatureHeader signatureHeader = getRequestHeader(headers);
        if (isPartnerModeV3()) {
            WxPayPartnerRefundNotifyV3Result response = client.parsePartnerRefundNotifyV3Result(body, signatureHeader);
            WxPayPartnerRefundNotifyV3Result.DecryptNotifyResult result = response.getResult();
            // 2. 构建结果
            if (Objects.equals("SUCCESS", result.getRefundStatus())) {
    ...
    private PayRefundRespDTO doGetRefundV3(String outTradeNo, String outRefundNo) throws WxPayException {
        ...
        // 2.1 执行请求
        WxPayRefundQueryV3Result response = isPartnerModeV3() ? client.refundPartnerQueryV3(request) : client.refundQueryV3(request);
        // 2.2 创建返回结果
