...
public interface PayClient<Config> {
     ...
     *
     * 说明：
     * 1. 默认返回 true，表示该支付客户端不额外执行渠道关单。
     * 2. 需要严格关单能力的客户端（如微信）应覆盖该方法并调用渠道关单 API。
     *
     * @param outTradeNo 外部订单号
     * @return 是否关单成功
     */
    default boolean closeOrder(String outTradeNo) {
        return true;
    }

    /**
     * 支付渠道回调处理成功时，HTTP 应答状态码
     *
     * @return HTTP 状态码
     */
    default int getNotifySuccessHttpStatus() {
        return 200;
    }

    /**
     * 支付渠道回调处理成功时，HTTP 应答内容
     *
     * @return 应答内容
     */
    default String getNotifySuccessBody() {
        return "success";
    }
