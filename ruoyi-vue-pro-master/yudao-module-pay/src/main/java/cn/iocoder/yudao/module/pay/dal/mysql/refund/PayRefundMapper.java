...
public interface PayRefundMapper extends BaseMapperX<PayRefundDO> {
    ...
    default PayRefundDO selectFirstByMerchantOrderId(String merchantOrderId) {
        return selectOne(new LambdaQueryWrapperX<PayRefundDO>()
                .eq(PayRefundDO::getMerchantOrderId, merchantOrderId)
                .orderByDesc(PayRefundDO::getId)
                .last("LIMIT 1"));
    }

    default PayRefundDO selectFirstByMerchantRefundId(String merchantRefundId) {
        return selectOne(new LambdaQueryWrapperX<PayRefundDO>()
                .eq(PayRefundDO::getMerchantRefundId, merchantRefundId)
                .orderByDesc(PayRefundDO::getId)
                .last("LIMIT 1"));
