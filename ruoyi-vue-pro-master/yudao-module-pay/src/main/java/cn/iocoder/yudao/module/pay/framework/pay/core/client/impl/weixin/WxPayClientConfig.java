...
public class WxPayClientConfig implements PayClientConfig {
     ...
     * 是否启用服务商模式（仅 V3 生效）
     *
     * true：按服务商模式请求微信支付 API
     * false：按普通商户模式请求；但如果配置了 subMchId，仍会自动按服务商模式处理
     */
    ...
    /**
     * 子商户 appId（服务商模式下可选；若在服务商小程序调起支付可不填）
     */
    ...
    @AssertTrue(message = "服务商模式下，子商户号不能为空", groups = V3.class)
    public boolean isPartnerConfigValid() {
        return !isPartnerMode() || hasText(subMchId);
    }
    ...
    public boolean isPartnerMode() {
        return Boolean.TRUE.equals(partnerMode) || hasText(subMchId);
    }
