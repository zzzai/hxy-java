...
public class PayNotifyController {
    ...
    @PermitAll
    @TenantIgnore
    public ResponseEntity<String> notifyOrder(@PathVariable("channelId") Long channelId,
        ...
        // 3. 先应答，再异步处理业务，降低回调超时风险
        submitNotifyTask("order", notify.getOutTradeNo(), () -> orderService.notifyOrder(channelId, notify));
        return buildNotifySuccessResponse(payClient);
    }
    ...
    @PermitAll
    @TenantIgnore
    public ResponseEntity<String> notifyRefund(@PathVariable("channelId") Long channelId,
                                               @RequestParam(required = false) Map<String, String> params,
                                               @RequestBody(required = false) String body,
                                               @RequestHeader Map<String, String> headers) {
        ...
        // 3. 先应答，再异步处理业务，降低回调超时风险
        submitNotifyTask("refund", notify.getOutRefundNo(), () -> refundService.notifyRefund(channelId, notify));
        return buildNotifySuccessResponse(payClient);
    }
    ...
    @PermitAll
    @TenantIgnore
    public ResponseEntity<String> notifyTransfer(@PathVariable("channelId") Long channelId,
                                                 @RequestParam(required = false) Map<String, String> params,
                                                 @RequestBody(required = false) String body,
                                                 @RequestHeader Map<String, String> headers) {
        ...
        // 3. 先应答，再异步处理业务，降低回调超时风险
        submitNotifyTask("transfer", notify.getOutTransferNo(), () -> payTransferService.notifyTransfer(channelId, notify));
        return buildNotifySuccessResponse(payClient);
    }

    private ResponseEntity<String> buildNotifySuccessResponse(PayClient<?> payClient) {
        int status = payClient.getNotifySuccessHttpStatus();
        if (status == 204) {
            return ResponseEntity.status(status).build();
        }
        return ResponseEntity.status(status).body(payClient.getNotifySuccessBody());
    }

    private void submitNotifyTask(String bizType, String bizNo, Runnable task) {
        try {
            notifyThreadPoolTaskExecutor.execute(() -> {
                try {
                    task.run();
                } catch (Throwable e) {
                    log.error("[submitNotifyTask][bizType({}) bizNo({}) 异步处理失败]", bizType, bizNo, e);
                }
            });
        } catch (RejectedExecutionException e) {
            log.error("[submitNotifyTask][bizType({}) bizNo({}) 线程池拒绝，降级同步执行]", bizType, bizNo, e);
            try {
                task.run();
            } catch (Throwable ex) {
                log.error("[submitNotifyTask][bizType({}) bizNo({}) 同步降级处理失败]", bizType, bizNo, ex);
            }
        }
    }
