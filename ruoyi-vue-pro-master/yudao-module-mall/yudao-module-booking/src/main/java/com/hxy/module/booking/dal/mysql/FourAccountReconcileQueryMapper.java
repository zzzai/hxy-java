...
public interface FourAccountReconcileQueryMapper {
    ...
    @Select({
            ...
            "  af.pay_refund_id AS payRefundId,",
            "  af.refund_time AS refundTime,",
            "  af.refund_limit_source AS refundLimitSource,",
            "  af.refund_limit_detail_json AS refundLimitDetailJson",
            "FROM trade_order t",
            "LEFT JOIN (",
            "  SELECT",
            "    order_id,",
            ...
            "LEFT JOIN (",
            "  SELECT latest.order_id, latest.pay_refund_id, latest.refund_time, latest.refund_limit_source, latest.refund_limit_detail_json",
            "  FROM trade_after_sale latest",
            "  INNER JOIN (",
            "    SELECT order_id, MAX(id) AS max_id",
            "    FROM trade_after_sale",
            "    WHERE deleted = 0",
            "    GROUP BY order_id",
            "  ) latest_id ON latest.id = latest_id.max_id",
            ") af ON af.order_id = t.id",
            "WHERE t.deleted = 0",
            "  AND t.pay_status = 1",
