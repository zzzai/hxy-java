...
public class AfterSaleController {
    ...
    public CommonResult<Boolean> refundAfterSale(@RequestParam("id") Long id,
                                                 @RequestParam(value = "forcePass", required = false, defaultValue = "false")
                                                 Boolean forcePass) {
        Long userId = getLoginUserId();
        AfterSaleDO afterSale = afterSaleService.getAfterSale(id);
        afterSaleRefundDecisionService.checkAndAuditForExecution(
                userId, UserTypeEnum.ADMIN.getValue(), afterSale, Boolean.TRUE.equals(forcePass));
        afterSaleService.refundAfterSale(userId, getClientIP(), id);
        return success(true);
    }
