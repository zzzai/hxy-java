...
public interface TradeOrderMapper extends BaseMapperX<TradeOrderDO> {
    ...
    default Long selectSumPayPriceByUserId(Long userId) {
        List<Map<String, Object>> result = selectMaps(new QueryWrapper<TradeOrderDO>()
                .select("IFNULL(SUM(pay_price), 0) AS sumPayPrice")
                .eq("user_id", userId)
                .eq("pay_status", 1));
        return MapUtil.getLong(CollUtil.getFirst(result), "sumPayPrice", 0L);
    }
    ...
    default TradeOrderDO selectFirstByNoAndUserId(String no, Long userId) {
        return selectOne(new LambdaQueryWrapperX<TradeOrderDO>()
                .eq(TradeOrderDO::getNo, no)
