...
# CRMEB 兼容层灰度阶段（可选）：full-compat | payment-core-only | disabled
CRMEB_COMPAT_STAGE=${CRMEB_COMPAT_STAGE:-}

function applyCrmebCompatStageProfile() {
    if [ -z "$CRMEB_COMPAT_STAGE" ]; then
        return
    fi
    ...
    case "$CRMEB_COMPAT_STAGE" in
        ...
        disabled)
            ...
            ;;
        *)
            echo "[stage] 不支持的 CRMEB_COMPAT_STAGE: $CRMEB_COMPAT_STAGE"
            echo "[stage] 允许值: full-compat | payment-core-only | disabled"
            exit 1
            ;;
    esac

    if [[ ",$PROFILES_ACTIVE," != *",$STAGE_PROFILE,"* ]]; then
        PROFILES_ACTIVE="$PROFILES_ACTIVE,$STAGE_PROFILE"
    fi
}
# 健康检查 URL
