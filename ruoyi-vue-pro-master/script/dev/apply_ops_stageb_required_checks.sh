...
usage() {
  cat <<'USAGE'
Usage:
  script/dev/apply_ops_stageb_required_checks.sh [options]
...
print_plan() {
  cat <<'PLAN'
[ops-stageb-checks] target contexts:
...
print_protection_summary() {
  ...
  if ! command -v gh >/dev/null 2>&1; then
    echo "[ops-stageb-checks] summary=SKIP (gh not found)"
    return 0
  fi
  ...
  echo "[ops-stageb-checks] branch_protection_summary:"
  set +e
  "${gh_cmd[@]}" --jq '.required_status_checks.contexts[]' | sed 's/^/  - /'
  local rc=$?
  set -e
  if [[ "${rc}" != "0" ]]; then
    echo "[ops-stageb-checks] summary=FAIL (cannot read branch protection)" >&2
    return 1
  fi
  echo "[ops-stageb-checks] summary=PASS"
}
...
print_plan
echo "[ops-stageb-checks] repo=${OWNER}/${REPO} branch=${BRANCH}"
echo "[ops-stageb-checks] step=dry-run"
bash "${SETUP_SCRIPT}" \
  --repo-owner "${OWNER}" \
  --repo-name "${REPO}" \
  --branch "${BRANCH}" \
...
echo "[ops-stageb-checks] step=apply"
bash "${SETUP_SCRIPT}" \
  --repo-owner "${OWNER}" \
  --repo-name "${REPO}" \
  --branch "${BRANCH}" \
