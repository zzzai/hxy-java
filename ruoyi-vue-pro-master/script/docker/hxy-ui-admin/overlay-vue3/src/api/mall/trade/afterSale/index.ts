...
export interface TradeAfterSaleVO {
  ...
  refundLimitSource?: string // 退款上限来源
  refundLimitSourceLabel?: string // 退款上限来源标签
...
export interface TradeAfterSalePageReqVO extends PageParam {
  userId?: number | null
  no?: string
...
// 获得交易售后分页
export const getAfterSalePage = async (params: TradeAfterSalePageReqVO) => {
  return await request.get({ url: `/trade/after-sale/page`, params })
}

// 获得交易售后详情
export const getAfterSale = async (id: any) => {
  return await request.get<TradeAfterSaleVO>({ url: `/trade/after-sale/get-detail?id=${id}` })
}
