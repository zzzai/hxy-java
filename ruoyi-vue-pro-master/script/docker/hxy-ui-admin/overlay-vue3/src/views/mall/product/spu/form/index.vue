...
const resolveSpuBaseUrl = () => {
  const productType = resolveRouteProductType()
  if (productType === 2) {
    return '/product/service-spu'
  }
  if (productType === 1) {
    return '/product/physical-spu'
  }
  return '/product/spu'
}
  ...
  if (id) {
    ...
    try {
      const res = (await request.get({
        url: `${resolveSpuBaseUrl()}/get-detail?id=${id}`
...
const submitForm = async () => {
  ...
  try {
    ...
    const routeProductType = resolveRouteProductType()
    if (routeProductType) {
      ;(data as any).productType = routeProductType
    }
    const id = params.id as unknown as number
    if (!id) {
      await request.post({ url: `${resolveSpuBaseUrl()}/create`, data })
      message.success(t('common.createSuccess'))
    } else {
      await request.put({ url: `${resolveSpuBaseUrl()}/update`, data })
...
const close = () => {
  ...
  const routeProductType = resolveRouteProductType()
  if (routeProductType === 2) {
    push({ path: '/mall/product/service-spu' })
    return
  }
  if (routeProductType === 1) {
    push({ path: '/mall/product/physical-spu' })
    return
  }
