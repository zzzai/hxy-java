...
const resolveSpuRuntime = () => {
  const path = route.path || ''
  if (path.includes('/mall/product/service-spu')) {
    return {
      baseUrl: '/product/service-spu',
      permissionPrefix: 'product:service-spu'
    }
  }
  if (path.includes('/mall/product/physical-spu') || path.includes('/mall/product/goods-spu')) {
    return {
      baseUrl: '/product/physical-spu',
      permissionPrefix: 'product:physical-spu'
    }
  }
  return {
    baseUrl: '/product/spu',
    permissionPrefix: 'product:spu'
  }
}
const spuRuntime = computed(resolveSpuRuntime)
const permissionPrefix = computed(() => spuRuntime.value.permissionPrefix)

const resolveRouteProductType = (): number | undefined => {
  const path = route.path || ''
  if (path.includes('/mall/product/service-spu')) {
    return 2
  }
...
const getList = async () => {
  loading.value = true
  try {
    const data = await request.get({ url: `${spuRuntime.value.baseUrl}/page`, params: queryParams.value })
...
const getTabsCount = async () => {
  const res = await request.get({ url: `${spuRuntime.value.baseUrl}/get-count` })
...
const handleStatus02Change = async (row: any, newStatus: number) => {
  try {
    ...
    // 发起修改
    await request.put({ url: `${spuRuntime.value.baseUrl}/update-status`, data: { id: row.id, status: newStatus } })
...
const handleStatusChange = async (row: any) => {
  try {
    ...
    // 发起修改
    await request.put({ url: `${spuRuntime.value.baseUrl}/update-status`, data: { id: row.id, status: row.status } })
    message.success(text + '成功')
    // 刷新 tabs 数据
    await getTabsCount()
    // 刷新列表
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await request.delete({ url: `${spuRuntime.value.baseUrl}/delete?id=${id}` })
...
/** 重置按钮操作 */
const resetQuery = () => {
  const productType = queryParams.value.productType
  queryFormRef.value.resetFields()
  queryParams.value.productType = productType
  handleQuery()
}

/** 新增或修改 */
const openForm = (id?: number) => {
  const pt = queryParams.value.productType
  // 修改
  if (typeof id === 'number') {
    push({ name: 'ProductSpuEdit', params: { id }, query: pt ? { pt } : {} })
    return
  }
  // 新增
  push({ name: 'ProductSpuAdd', query: pt ? { pt } : {} })
}

/** 查看商品详情 */
const openDetail = (id: number) => {
  const pt = queryParams.value.productType
  push({ name: 'ProductSpuDetail', params: { id }, query: pt ? { pt } : {} })
}
...
/** 初始化 **/
onMounted(async () => {
  queryParams.value.productType = resolveRouteProductType()
  // 解析路由的 categoryId
