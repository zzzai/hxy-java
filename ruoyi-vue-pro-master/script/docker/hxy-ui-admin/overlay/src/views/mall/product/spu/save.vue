...
export default {
  ...
  created() {
    ...
    const routeProductType = Number(this.$route.query && this.$route.query.pt);
    if (!Number.isNaN(routeProductType) && routeProductType > 0) {
      this.baseForm.productType = routeProductType;
    }
  ...
  methods: {
    resolveBackPath() {
      if (this.baseForm.productType === 2) {
        return "/product/service-spu";
      }
      if (this.baseForm.productType === 1) {
        return "/product/goods-spu";
      }
      return "/product/spu";
    },
    removeSpec(index) {
    ...
    cancel() {
      ...
      this.$store.dispatch('tagsView/delView', currentView)
        .then(() => {
          this.$router.push(this.resolveBackPath())
        })
    },
    submit() {
    ...
    updateType(id) {
      getSpuDetail(id).then((response) => {
        ...
        this.baseForm.productType = data.productType || this.baseForm.productType;
