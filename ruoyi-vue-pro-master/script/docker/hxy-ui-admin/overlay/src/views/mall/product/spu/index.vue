...
export default {
  ...
  created() {
    this.initProductTypeByRoute();
    this.getList();
    this.getListCategory()
    this.getListBrand()
  },
  methods: {
    resolveRouteProductType() {
      const path = (this.$route && this.$route.path) || "";
      if (path.includes("/product/service-spu")) {
        return 2;
      }
      if (path.includes("/product/goods-spu")) {
        return 1;
      }
      return null;
    },
    initProductTypeByRoute() {
      this.queryParams.productType = this.resolveRouteProductType();
    },
    resolveTabType() {
      if (this.activeTabs === "on") {
        return 0;
      }
      if (this.activeTabs === "off") {
        return 1;
      }
      if (this.activeTabs === "remind") {
        return 3;
      }
      return null;
    },
    ...
    getList() {
      ...
      params.tabType = this.resolveTabType();
    ...
    resetQuery() {
      ...
      const productType = this.queryParams.productType;
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push({ name: 'ProductSpuCreate', query: { pt: this.queryParams.productType }})
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.$router.push({ name: 'ProductSpuUpdate', params: { spuId: row.id }, query: { pt: this.queryParams.productType }})
    },
    /** 删除按钮操作 */
    handleDelete(row) {
      const id = row.id;
