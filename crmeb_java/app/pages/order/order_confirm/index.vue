		...
		methods: {
			...
			getloadPreOrder: function() {
				loadPreOrderApi(this.preOrderNo).then(res => {
					...
					this.cartArr[1].payStatus = this.switchOn(res.data.yuePayStatus) ? 1 : 2;
					this.cartArr[0].payStatus = this.switchOn(res.data.payWeixinOpen) ? 1 : 0;
					this.getaddressInfo();
					// #ifdef H5
					if (this.$wechat.isWeixin()) this.cartArr.pop();
					// #endif
					this.store_self_mention = this.switchOn(res.data.storeSelfMention) && this
						.productType ===
						'normal' ? true : false;
				}).catch(err => {
					uni.navigateTo({
