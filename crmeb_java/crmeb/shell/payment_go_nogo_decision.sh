...
usage() {
  cat <<'USAGE'
用法：
  ./shell/payment_go_nogo_decision.sh [--date YYYY-MM-DD] [--order-no ORDER_NO] [--ticket-summary PATH] [--require-apply-ready 0|1] [--require-booking-repair-pass 0|1] [--require-mapping-smoke-green 0|1] [--no-alert] [--out-dir PATH]
...
参数：
  ...
  --require-mapping-smoke-green 0|1  是否要求 latest store_mapping_smoke overall=GREEN（默认 0）
  --no-alert                   不推送机器人告警
...
echo "[go-nogo] report_date=${REPORT_DATE}, order_no=${ORDER_NO:-<none>}, require_apply_ready=${REQUIRE_APPLY_READY}, require_booking_repair_pass=${REQUIRE_BOOKING_REPAIR_PASS}, require_mapping_smoke_green=${REQUIRE_MAPPING_SMOKE_GREEN}"
...
# 1.7) store_mapping_smoke（可选门槛）
mapping_smoke_summary="$(latest_summary "${ROOT_DIR}/runtime/payment_store_mapping_smoke")"
mapping_smoke_overall="$(kv "${mapping_smoke_summary}" "overall")"
mapping_smoke_gate_ok=1
if [[ "${REQUIRE_MAPPING_SMOKE_GREEN}" == "1" ]]; then
  if [[ -z "${mapping_smoke_summary}" ]]; then
    mapping_smoke_gate_ok=0
    blockers+=("缺少 store_mapping_smoke summary（已要求 GREEN）")
  elif [[ "${mapping_smoke_overall}" != "GREEN" ]]; then
    mapping_smoke_gate_ok=0
    blockers+=("store_mapping_smoke 未通过(overall=${mapping_smoke_overall:-N/A})")
  fi
fi
...
if [[ "${rehearsal_ok}" == "1" && "${ticket_gate_ok}" == "1" ]]; then
  if [[ "${booking_repair_gate_ok}" == "1" ]]; then
    if [[ "${mapping_smoke_gate_ok}" == "1" ]]; then
      if [[ "${REQUIRE_APPLY_READY}" == "1" ]]; then
...

{
  ...
  echo "mapping_smoke_gate_ok=${mapping_smoke_gate_ok}"
  echo "mapping_smoke_overall=${mapping_smoke_overall}"
...

{
  ...
  echo "| store_mapping_smoke(latest) | $([[ ${mapping_smoke_gate_ok} -eq 1 ]] && echo PASS || echo FAIL) | overall=${mapping_smoke_overall:-N/A} |"
