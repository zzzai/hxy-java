...
preflight_raw_rc="$(run_step "01_preflight" "${preflight_args[@]}")"
preflight_rc="${preflight_raw_rc}"
# 非 strict 预检下，rc=2 仅表示存在 WARN，不阻断预演通过。
if [[ ${STRICT_PREFLIGHT} -eq 0 && "${preflight_raw_rc}" == "2" ]]; then
  preflight_rc="0"
fi
...
preflight_raw_rc=${preflight_raw_rc}
...
| preflight | ${preflight_raw_rc} | strict预检（gate_rc=${preflight_rc}） |
...
if [[ "${ready}" != "1" ]]; then
  if [[ ${NO_ALERT} -eq 0 ]] && [[ -x "${ALERT_SCRIPT}" ]]; then
    "${ALERT_SCRIPT}" \
      --title "支付切换预演未通过" \
      --content "ready_for_order_drill=${ready}; snapshot=${snapshot_raw_rc}; preflight_raw=${preflight_raw_rc}; preflight_gate=${preflight_rc}; ops_daily=${ops_daily_rc}; idempotency=${idempotency_rc}; ticketize=${ticketize_raw_rc}; mapping=${mapping_raw_rc}; retention=${retention_raw_rc}; report=${REPORT_FILE}" || true
  fi
  exit 2
fi
