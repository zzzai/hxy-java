...
# 支付早班一键包：
# ops_daily -> go_nogo -> warroom -> booking_verify_regression -> booking_verify_repair -> refund_convergence
...
usage() {
  cat <<'USAGE'
用法：
  ./shell/payment_ops_morning_bundle.sh [--date YYYY-MM-DD] [--order-no ORDER_NO] [--window N] [--tail N] [--window-hours N] [--refund-window-hours N] [--refund-timeout-minutes N] [--require-apply-ready 0|1] [--require-booking-repair-pass 0|1] [--require-mapping-smoke-green 0|1] [--sla-lookback-days N] [--sla-days N] [--strict-preflight] [--no-alert] [--out-dir PATH]

参数：
  --date YYYY-MM-DD            对账/看板日期（默认昨天）
  --order-no ORDER_NO          透传给 go_nogo 的订单号（可选）
  ...
  --require-mapping-smoke-green 0|1  go_nogo 是否要求 store_mapping_smoke=GREEN（默认 0）
...
if [[ "${REQUIRE_MAPPING_SMOKE_GREEN}" != "0" && "${REQUIRE_MAPPING_SMOKE_GREEN}" != "1" ]]; then
  echo "参数错误: --require-mapping-smoke-green 仅支持 0 或 1"
  exit 1
fi
...
echo "[morning-bundle] date=${REPORT_DATE}, order_no=${ORDER_NO:-<none>}, window=${WINDOW_MINUTES}, tail=${TAIL_LINES}, window_hours=${WINDOW_HOURS}, require_booking_repair_pass=${REQUIRE_BOOKING_REPAIR_PASS}, require_mapping_smoke_green=${REQUIRE_MAPPING_SMOKE_GREEN}, sla_lookback_days=${SLA_LOOKBACK_DAYS}, sla_days=${SLA_DAYS}, refund_window_hours=${REFUND_WINDOW_HOURS}, refund_timeout_minutes=${REFUND_TIMEOUT_MINUTES}"
...
gonogo_cmd=(
  ...
  --date "${REPORT_DATE}"
  --require-apply-ready "${REQUIRE_APPLY_READY}"
  --require-booking-repair-pass "${REQUIRE_BOOKING_REPAIR_PASS}"
  --require-mapping-smoke-green "${REQUIRE_MAPPING_SMOKE_GREEN}"
  --no-alert
)
if [[ -n "${ORDER_NO}" ]]; then
  gonogo_cmd+=(--order-no "${ORDER_NO}")
fi
gonogo_rc="$(run_step "02_go_nogo" "${gonogo_cmd[@]}")"
...
refund_rc="$(run_step "07_refund_convergence" ./shell/payment_refund_convergence_check.sh --window-hours "${REFUND_WINDOW_HOURS}" --refund-timeout-minutes "${REFUND_TIMEOUT_MINUTES}" --out-dir "${RUN_DIR}/refund_convergence" --no-alert)"
refund_summary="$(find "${RUN_DIR}/refund_convergence" -maxdepth 2 -type f -name summary.txt 2>/dev/null | sort | tail -n 1 || true)"
refund_gate_result="$(kv "${refund_summary}" "gate_result")"
refund_block_check_count="$(kv "${refund_summary}" "block_check_count")"
refund_warn_check_count="$(kv "${refund_summary}" "warn_check_count")"

severity="OK"
exit_code=0
if [[ "${ops_rc}" == "1" || "${gonogo_rc}" == "1" || "${warroom_rc}" == "1" || "${sla_rc}" == "1" || "${booking_verify_rc}" == "1" || "${booking_repair_rc}" == "1" || "${refund_rc}" == "1" ]]; then
  severity="ERROR"
  exit_code=1
elif [[ "${ops_rc}" == "2" || "${gonogo_rc}" == "2" || "${warroom_rc}" == "2" || "${sla_rc}" == "2" || "${booking_verify_rc}" == "2" || "${booking_repair_rc}" == "2" || "${refund_rc}" == "2" ]]; then
  severity="ALERT"
  exit_code=2
fi
...
require_mapping_smoke_green=${REQUIRE_MAPPING_SMOKE_GREEN}
...
refund_gate_result=${refund_gate_result}
refund_block_check_count=${refund_block_check_count}
refund_warn_check_count=${refund_warn_check_count}
run_dir=${RUN_DIR}
TXT
...
| refund_convergence | ${refund_rc} | gate_result=${refund_gate_result:-N/A}, block_checks=${refund_block_check_count:-N/A}, warn_checks=${refund_warn_check_count:-N/A} |
...
echo "[morning-bundle] severity=${severity}, ops_rc=${ops_rc}, gonogo_rc=${gonogo_rc}, warroom_rc=${warroom_rc}, sla_rc=${sla_rc}, booking_verify_rc=${booking_verify_rc}, booking_repair_rc=${booking_repair_rc}, refund_rc=${refund_rc}"

if (( exit_code != 0 )) && [[ ${NO_ALERT} -eq 0 ]] && [[ -x "${ALERT_SCRIPT}" ]]; then
  "${ALERT_SCRIPT}" \
    --title "支付早班一键包告警" \
    --content "severity=${severity}; date=${REPORT_DATE}; ops_rc=${ops_rc}; go_nogo_rc=${gonogo_rc}; warroom_rc=${warroom_rc}; sla_rc=${sla_rc}; booking_verify_rc=${booking_verify_rc}; booking_repair_rc=${booking_repair_rc}; refund_rc=${refund_rc}; report=${REPORT_FILE}" || true
fi
