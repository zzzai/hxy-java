...
# 输入兼容：
# - 业务订单号（eb_store_order.order_id）
# - 商户单号（eb_store_order.out_trade_no）
# - 微信交易单号（eb_wechat_pay_info.transaction_id）
...
参数：
  --order-no   指定演练标识（支持 order_id / out_trade_no / transaction_id）
...
input_no="${ORDER_NO}"
input_esc="$(sql_escape "${input_no}")"
resolved_by="order_id"
resolved_input="${input_no}"
...
if [[ -z "${order_row}" ]]; then
  order_row="$("${MYSQL_CMD[@]}" -e "
    SELECT so.id, so.order_id, IFNULL(so.out_trade_no,''), so.paid, IFNULL(so.refund_status,0), so.pay_price, DATE_FORMAT(so.pay_time,'%Y-%m-%d %H:%i:%s'), IFNULL(so.store_id,0)
    FROM eb_store_order so
    WHERE so.out_trade_no='${input_esc}'
    LIMIT 1;")"
  if [[ -n "${order_row}" ]]; then
    resolved_by="out_trade_no"
    resolved_input="${input_no}"
  fi
fi

if [[ -z "${order_row}" ]]; then
  derived_out_trade_no="$("${MYSQL_CMD[@]}" -e "
    SELECT IFNULL(p.out_trade_no,'')
    FROM eb_wechat_pay_info p
    WHERE p.transaction_id='${input_esc}' OR p.out_trade_no='${input_esc}'
    ORDER BY p.id DESC
    LIMIT 1;")"
  if [[ -n "${derived_out_trade_no}" ]]; then
    derived_out_trade_no_esc="$(sql_escape "${derived_out_trade_no}")"
    order_row="$("${MYSQL_CMD[@]}" -e "
      SELECT so.id, so.order_id, IFNULL(so.out_trade_no,''), so.paid, IFNULL(so.refund_status,0), so.pay_price, DATE_FORMAT(so.pay_time,'%Y-%m-%d %H:%i:%s'), IFNULL(so.store_id,0)
      FROM eb_store_order so
      WHERE so.out_trade_no='${derived_out_trade_no_esc}'
      LIMIT 1;")"
    if [[ -n "${order_row}" ]]; then
      resolved_by="transaction_id"
      resolved_input="${input_no}"
    fi
  fi
fi
...
if [[ -z "${order_row}" ]]; then
  echo "订单不存在: ${input_no}（已按 order_id/out_trade_no/transaction_id 三种方式反查）"
  echo "提示：当前连接 DB=${DB_NAME}@${DB_HOST}:${DB_PORT}，如使用 Docker MySQL 请设置 DB_PORT=33306 或改用 MYSQL_DEFAULTS_FILE。"
  exit 1
fi
