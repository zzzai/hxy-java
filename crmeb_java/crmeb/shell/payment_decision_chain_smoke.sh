...
usage() {
  cat <<'USAGE'
用法：
  ./shell/payment_decision_chain_smoke.sh [--date YYYY-MM-DD] [--go-nogo-require-mapping-smoke-green 0|1] [--keep-temp] [--out-dir PATH]

参数：
  --date YYYY-MM-DD    业务日期（默认昨天）
  --go-nogo-require-mapping-smoke-green 0|1  go_nogo 是否要求 mapping_smoke=GREEN（默认 0）
...
TXT
  ...
  touch_summary "${TMP_RUNTIME}/payment_store_mapping_smoke/run-green/summary.txt" <<TXT
run_id=green
overall=GREEN
run_dir=${TMP_RUNTIME}/payment_store_mapping_smoke/run-green
TXT
...
bash "${TMP_SHELL}/payment_go_nogo_decision.sh" \
  ...
  --require-mapping-smoke-green "${GONOGO_REQUIRE_MAPPING_SMOKE_GREEN}" \
  --no-alert \
...
GO_MAPPING_SMOKE_GATE_OK="$(kv "${GO_SUMMARY}" "mapping_smoke_gate_ok")"
GO_MAPPING_SMOKE_OVERALL="$(kv "${GO_SUMMARY}" "mapping_smoke_overall")"
...
if [[ "${GONOGO_REQUIRE_MAPPING_SMOKE_GREEN}" == "1" && "${GO_MAPPING_SMOKE_GATE_OK}" != "1" ]]; then
  fails+=("go_nogo mapping_smoke_gate_ok=${GO_MAPPING_SMOKE_GATE_OK:-N/A}（预期 1）")
fi
...
go_nogo_require_mapping_smoke_green=${GONOGO_REQUIRE_MAPPING_SMOKE_GREEN}
go_nogo_rc=${GO_RC}
...
go_nogo_mapping_smoke_gate_ok=${GO_MAPPING_SMOKE_GATE_OK}
go_nogo_mapping_smoke_overall=${GO_MAPPING_SMOKE_OVERALL}
warroom_rc=${WAR_RC}
...
| go_nogo mapping_smoke_gate_ok | ${GO_MAPPING_SMOKE_GATE_OK:-N/A} | $([[ "${GONOGO_REQUIRE_MAPPING_SMOKE_GREEN}" == "1" ]] && echo 1 || echo N/A) |
| go_nogo mapping_smoke_overall | ${GO_MAPPING_SMOKE_OVERALL:-N/A} | $([[ "${GONOGO_REQUIRE_MAPPING_SMOKE_GREEN}" == "1" ]] && echo GREEN || echo N/A) |
