...
usage() {
  cat <<'USAGE'
用法：
  ./shell/payment_store_mapping_audit.sh [--out-dir PATH] [--runtime-root PATH] [--include-deleted 0|1] [--strict-missing 0|1] [--allow-shared-submchid 0|1] [--no-alert]
...
参数：
  ...
  --allow-shared-submchid 0|1 是否允许多个门店复用同一 sub_mchid（默认 0，不允许）
  --no-alert              风险时不推送机器人
...
if [[ "${ALLOW_SHARED_SUBMCHID}" != "0" && "${ALLOW_SHARED_SUBMCHID}" != "1" ]]; then
  echo "参数错误: --allow-shared-submchid 仅支持 0 或 1"
  exit 1
fi
...
PLACEHOLDER_FILE="${RUN_DIR}/placeholder.tsv"
...
while IFS=$'\t' read -r store_id store_name store_phone store_is_del sub_mchid; do
  ...
  if [[ "${sub_mchid}" =~ ^99[0-9]{8}$ ]]; then
    STORE_PLACEHOLDER["${store_id}"]=1
...
for store_id in $(printf '%s\n' "${!STORE_SUB[@]}" | sort -n); do
  ...
  if [[ -n "${STORE_PLACEHOLDER["${store_id}"]-}" ]]; then
    if [[ "${status}" == "OK" ]]; then
      status="PLACEHOLDER"
      reason="sub_mchid 为占位号(99xxxxxxxx)"
    else
      status="${status}+PLACEHOLDER"
      reason="${reason}; sub_mchid 为占位号(99xxxxxxxx)"
    fi
    printf "%s\t%s\t%s\t%s\n" "${store_id}" "${store_name}" "${sub_mchid}" "sub_mchid 为占位号(99xxxxxxxx)" >> "${PLACEHOLDER_FILE}"
  fi
...
duplicate_critical_count=0
if [[ "${ALLOW_SHARED_SUBMCHID}" == "0" ]]; then
  duplicate_critical_count="${duplicate_store_count}"
fi
critical_count=$((invalid_count + duplicate_critical_count + orphan_non_empty_count + placeholder_count))
...

{
  ...
  echo "placeholder_file=${PLACEHOLDER_FILE}"
