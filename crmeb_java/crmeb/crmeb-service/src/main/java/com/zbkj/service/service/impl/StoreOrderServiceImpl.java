...
//    private WechatOrderShippingService wechatOrderShippingService;
    ...
    */
    ...
    public CommonPage<StoreOrderDetailResponse> getAdminList(StoreOrderSearchRequest request, PageParamRequest pageParamRequest) {
        ...
        if (Boolean.TRUE.equals(request.getRefundTicketOnly())) {
            queryWrapper.apply("exists (select 1 from eb_store_order_status sos where sos.oid = eb_store_order.id and sos.change_type = {0})",
                    Constants.ORDER_LOG_REFUND_TICKET);
        }
...
//        HashMap<Integer, SystemStoreStaff> systemStoreStaffList = systemStoreStaffService.getMapInId(clerkIdList);
    ...
    private boolean doRefund(StoreOrderRefundRequest request) {
        ...
        //退款
        boolean refundSubmitUncertain = false;
        String refundSubmitMessage = "";
        if (storeOrder.getPayType().equals(Constants.PAY_TYPE_WE_CHAT) && request.getAmount().compareTo(BigDecimal.ZERO) > 0) {
            try {
                ...
                String errorMsg = sanitizeRefundErrorMsg(e.getMessage());
                if (isRefundSubmitUncertain(errorMsg)) {
                    refundSubmitUncertain = true;
                    refundSubmitMessage = errorMsg;
                    logger.warn("微信退款提交结果不确定，转入补偿链路，orderNo={}, outTradeNo={}, msg={}",
                            storeOrder.getOrderId(), storeOrder.getOutTradeNo(), errorMsg);
                } else {
                    storeOrderStatusService.saveRefund(storeOrder.getId(), request.getAmount(), "失败(" + errorMsg + ")");
                    throw new CrmebException("微信申请退款失败：" + errorMsg);
                }
            }
        }
        //修改订单退款状态
        storeOrder.setRefundStatus(3);
        storeOrder.setRefundPrice(storeOrder.getRefundPrice().add(request.getAmount()));
        ...
        if (refundSubmitUncertain) {
            storeOrderStatusService.saveRefund(storeOrder.getId(), request.getAmount(),
                    "提交不确定，进入补偿(" + sanitizeRefundErrorMsg(refundSubmitMessage) + ")");
        }
        return execute;
    }

    private boolean isRefundSubmitUncertain(String msg) {
        String upper = StrUtil.blankToDefault(msg, "").toUpperCase();
        return upper.contains("XMLTOMAP")
                || upper.contains("通信异常")
                || upper.contains("连接")
                || upper.contains("超时")
                || upper.contains("NO HTTP RESOURCE")
                || upper.contains("NO RESPONSE")
                || upper.contains("微信无信息返回")
                || upper.contains("HTTPSTATUS=5");
    }
