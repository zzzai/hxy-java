...
public class OrderServiceImpl implements OrderService {
    ...
    private static final int REFUND_AUTO_MAX_MINUTES_DEFAULT = 120;
    private static final BigDecimal REFUND_AUTO_MAX_AMOUNT_DEFAULT = new BigDecimal("200.00");
    private static final String REFUND_MANUAL_KEYWORDS_DEFAULT = "洗钱,套现,盗刷,诈骗,非法";
    ...
    public Boolean refundApply(OrderRefundApplyRequest request) {
        ...
        String userReason = StrUtil.blankToDefault(request.getExplain(), request.getText());

        existStoreOrder.setUpdateTime(DateUtil.date());
        Boolean execute = transactionTemplate.execute(e -> {
            storeOrderService.updateById(existStoreOrder);
            storeOrderStatusService.createLog(existStoreOrder.getId(), Constants.ORDER_LOG_REFUND_APPLY, "用户申请退款原因：" + userReason);
            return Boolean.TRUE;
        });
        if (!execute) {
        ...
        RefundRouteDecision decision = evaluateRefundRoute(existStoreOrder, request);
        if (decision.autoPass) {
            try {
                StoreOrderRefundRequest refundRequest = new StoreOrderRefundRequest();
                refundRequest.setOrderNo(existStoreOrder.getOrderId());
                refundRequest.setAmount(existStoreOrder.getPayPrice());
                storeOrderStatusService.createLog(existStoreOrder.getId(), Constants.ORDER_LOG_REFUND_AUTO_SUBMIT,
                        "总部自动秒退已受理，命中规则：" + decision.reason);
                storeOrderService.refund(refundRequest);
                return true;
            } catch (Exception e) {
                String error = sanitizeRefundErrorMsg(e.getMessage());
                storeOrderStatusService.createLog(existStoreOrder.getId(), Constants.ORDER_LOG_REFUND_TICKET,
                        "自动秒退提交失败，转总部人工复核：" + error);
                notifyRefundApplyToAdmin(existStoreOrder);
                logger.warn("自动秒退提交失败，orderNo={}, msg={}", existStoreOrder.getOrderId(), error);
                return true;
            }
        }
        storeOrderStatusService.createLog(existStoreOrder.getId(), Constants.ORDER_LOG_REFUND_TICKET,
                "进入总部退款工单池，原因：" + decision.reason);
        notifyRefundApplyToAdmin(existStoreOrder);
        return true;
    }

    private void notifyRefundApplyToAdmin(StoreOrder storeOrder) {
        SystemNotification notification = systemNotificationService.getByMark(NotifyConstants.APPLY_ORDER_REFUND_ADMIN_MARK);
        if (ObjectUtil.isNull(notification) || !notification.getIsSms().equals(1)) {
            return;
        }
        List<SystemAdmin> systemAdminList = systemAdminService.findIsSmsList();
        if (CollUtil.isEmpty(systemAdminList)) {
    ...
    private RefundRouteDecision evaluateRefundRoute(StoreOrder order, OrderRefundApplyRequest request) {
        String autoEnableValue = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_PAY_REFUND_AUTO_ENABLE);
        if (!ConfigSwitchUtil.isOn(StrUtil.blankToDefault(autoEnableValue, "1"))) {
            return RefundRouteDecision.manual("自动秒退开关关闭");
        }
        if (ObjectUtil.isNull(order.getPayPrice()) || order.getPayPrice().compareTo(BigDecimal.ZERO) <= 0) {
            return RefundRouteDecision.manual("订单金额异常");
        }
        ...
        if (Constants.PAY_TYPE_WE_CHAT.equals(order.getPayType()) && StrUtil.isBlank(order.getOutTradeNo())) {
            return RefundRouteDecision.manual("缺少渠道商户单号");
        }
        BigDecimal maxAmount = getRefundAutoMaxAmount();
        if (order.getPayPrice().compareTo(maxAmount) > 0) {
            return RefundRouteDecision.manual("金额超过自动秒退上限");
        }
        int maxMinutes = getRefundAutoMaxMinutes();
        Date baseTime = ObjectUtil.isNotNull(order.getPayTime()) ? order.getPayTime() : order.getCreateTime();
        if (ObjectUtil.isNotNull(baseTime) && DateUtil.between(baseTime, DateUtil.date(), DateUnit.MINUTE) > maxMinutes) {
            return RefundRouteDecision.manual("超出自动秒退时效窗口");
        }
        if (hitManualRiskKeyword(request)) {
            return RefundRouteDecision.manual("命中高风险关键词");
        }
        return RefundRouteDecision.auto("金额/时效/履约状态均符合");
    }

    private boolean isSupportedAutoRefundPayType(String payType) {
        return Constants.PAY_TYPE_WE_CHAT.equals(payType)
                || Constants.PAY_TYPE_YUE.equals(payType)
                || Constants.PAY_TYPE_ALI_PAY.equals(payType);
    }

    private int getRefundAutoMaxMinutes() {
        String raw = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_PAY_REFUND_AUTO_MAX_MINUTES);
        if (StrUtil.isBlank(raw)) {
            return REFUND_AUTO_MAX_MINUTES_DEFAULT;
        }
        try {
            int value = Integer.parseInt(StrUtil.trim(raw));
            return value > 0 ? value : REFUND_AUTO_MAX_MINUTES_DEFAULT;
        } catch (Exception ignore) {
            return REFUND_AUTO_MAX_MINUTES_DEFAULT;
        }
    }

    private BigDecimal getRefundAutoMaxAmount() {
        String raw = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_PAY_REFUND_AUTO_MAX_AMOUNT);
        if (StrUtil.isBlank(raw)) {
            return REFUND_AUTO_MAX_AMOUNT_DEFAULT;
        }
        try {
            BigDecimal value = new BigDecimal(StrUtil.trim(raw));
            return value.compareTo(BigDecimal.ZERO) > 0 ? value : REFUND_AUTO_MAX_AMOUNT_DEFAULT;
        } catch (Exception ignore) {
            return REFUND_AUTO_MAX_AMOUNT_DEFAULT;
        }
    }

    private boolean hitManualRiskKeyword(OrderRefundApplyRequest request) {
        String configured = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_PAY_REFUND_MANUAL_KEYWORDS);
        String keywords = StrUtil.blankToDefault(configured, REFUND_MANUAL_KEYWORDS_DEFAULT);
        if (StrUtil.isBlank(keywords)) {
            return false;
        }
        String content = (StrUtil.blankToDefault(request.getText(), "") + " " + StrUtil.blankToDefault(request.getExplain(), "")).toLowerCase();
    ...
    private static class RefundRouteDecision {
        private final boolean autoPass;
        private final String reason;

        private RefundRouteDecision(boolean autoPass, String reason) {
            this.autoPass = autoPass;
            this.reason = reason;
        }

        private static RefundRouteDecision auto(String reason) {
            return new RefundRouteDecision(true, reason);
        }

        private static RefundRouteDecision manual(String reason) {
            return new RefundRouteDecision(false, reason);
        }
    }
    ...
    public PreOrderResponse loadPreOrder(String preOrderNo) {
        ...
        String payWeixinOpen = ConfigSwitchUtil.normalize01(systemConfigService.getValueByKey(SysConfigConstants.CONFIG_PAY_WEIXIN_OPEN));
        ...
        String yuePayStatus = ConfigSwitchUtil.normalize01(systemConfigService.getValueByKey(SysConfigConstants.CONFIG_YUE_PAY_STATUS));
        String storeSelfMention = ConfigSwitchUtil.normalize01(systemConfigService.getValueByKey(SysConfigConstants.CONFIG_KEY_STORE_SELF_MENTION));
        String aliPayStatus = ConfigSwitchUtil.normalize01(systemConfigService.getValueByKey(SysConfigConstants.CONFIG_ALI_PAY_STATUS));
