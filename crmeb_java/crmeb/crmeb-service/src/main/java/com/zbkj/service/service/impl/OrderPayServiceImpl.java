...
public class OrderPayServiceImpl implements OrderPayService {
    ...
    public PayConfigResponse getPayConfig() {
        ...
        boolean yueEnabled = ConfigSwitchUtil.isOn(yuePayStatus);
        response.setYuePayStatus(yueEnabled);
        response.setPayWechatOpen(ConfigSwitchUtil.isOn(payWxOpen));
        if (yueEnabled) {
...
//        temMap.put("thing5", "恭喜您拼团成功！我们将尽快为您发货。");
    ...
    public OrderPayResultResponse payment(OrderPayRequest orderPayRequest, String ip) {
        String lockKey = "pay:order:lock:" + orderPayRequest.getOrderNo();
        return distributedLockUtil.executeWithLock(lockKey, 15, () -> doPayment(orderPayRequest, ip));
    }

    private OrderPayResultResponse doPayment(OrderPayRequest orderPayRequest, String ip) {
    ...
    private Map<String, String> unifiedorder(StoreOrder storeOrder, String ip) {
        ...
        // 获取微信预下单对象
        CreateOrderRequestVo unifiedorderVo = getUnifiedorderVo(storeOrder, userToken.getToken(), ip, payConfig);
        reserveOutTradeNoIfNeeded(storeOrder, unifiedorderVo.getOut_trade_no());
        logger.info(
                "wx.unifiedorder.request biz=store_order orderNo={} outTradeNo={} channel={} storeId={} serviceProviderMode={} mchId={} subMchId={}",
                storeOrder.getOrderId(),
                unifiedorderVo.getOut_trade_no(),
                storeOrder.getIsChannel(),
        ...
        logger.info(
                "wx.unifiedorder.response biz=store_order orderNo={} outTradeNo={} channel={} storeId={} serviceProviderMode={} mchId={} subMchId={} prepayId={}",
                storeOrder.getOrderId(),
                unifiedorderVo.getOut_trade_no(),
                storeOrder.getIsChannel(),
                storeOrder.getStoreId(),
    ...
    private void reserveOutTradeNoIfNeeded(StoreOrder storeOrder, String outTradeNo) {
        if (StrUtil.isBlank(outTradeNo)) {
            throw new CrmebException("微信下单失败：out_trade_no为空");
        }
        if (StrUtil.isNotBlank(storeOrder.getOutTradeNo())) {
            return;
        }
        storeOrder.setOutTradeNo(outTradeNo);
        storeOrder.setUpdateTime(DateUtil.date());
        boolean updated = storeOrderService.updateById(storeOrder);
        if (!updated) {
            throw new CrmebException("微信下单失败：预占商户单号失败");
        }
    }
     ...
     * @return 微信预下单对象
     */
    private CreateOrderRequestVo getUnifiedorderVo(StoreOrder storeOrder, String openid, String ip, WeChatPayChannelConfig payConfig) {
        // 获取域名
        ...
        String outTradeNo = StrUtil.isNotBlank(storeOrder.getOutTradeNo())
                ? storeOrder.getOutTradeNo()
                : CrmebUtil.getOrderNo("wxNo");
