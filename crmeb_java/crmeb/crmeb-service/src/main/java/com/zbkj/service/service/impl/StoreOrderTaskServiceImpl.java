...
public class StoreOrderTaskServiceImpl implements StoreOrderTaskService {
    ...
    public Boolean refundOrder(StoreOrder storeOrder) {
        if (!OrderRefundStateMachine.shouldRunRefundTask(storeOrder.getRefundStatus())) {
            logger.warn("退款后置处理跳过，状态不匹配：orderId={}, refundStatus={}", storeOrder.getId(), storeOrder.getRefundStatus());
            return Boolean.TRUE;
        }
        if (OrderRefundStateMachine.isFinalSuccess(storeOrder.getRefundStatus()) && hasRefundSuccessRecord(storeOrder.getId())) {
            logger.info("退款后置处理幂等命中，已存在成功退款日志：orderId={}", storeOrder.getId());
            return Boolean.TRUE;
        }
        /**
         * 1、写订单日志
    ...
    /**
     * 是否已写入“退款成功”订单日志，用于幂等保护。
     */
    private boolean hasRefundSuccessRecord(Integer orderId) {
        LambdaQueryWrapper<StoreOrderStatus> lqw = new LambdaQueryWrapper<>();
        lqw.eq(StoreOrderStatus::getOid, orderId)
                .eq(StoreOrderStatus::getChangeType, Constants.ORDER_LOG_REFUND_PRICE)
                .like(StoreOrderStatus::getChangeMessage, "成功")
                .last("limit 1");
    ...
    public Boolean autoCancel(StoreOrder storeOrder) {
        ...
        if (!closeWechatOrderIfNeeded(storeOrder)) {
            return Boolean.FALSE;
        }
    ...
    /**
     * 超时取消前先尝试关掉微信侧订单，避免本地取消后仍可被支付。
     */
    private boolean closeWechatOrderIfNeeded(StoreOrder storeOrder) {
        ...
        if (StrUtil.isBlank(storeOrder.getOutTradeNo())) {
            logger.warn("autoCancel 跳过微信关单：outTradeNo为空，orderNo={}", storeOrder.getOrderId());
            return true;
        }
        // 关单前先查单，若已支付或结果不确定则先不执行本地取消，等待下一轮补偿。
        try {
            Boolean paid = weChatPayService.queryPayResult(storeOrder.getOrderId());
            if (Boolean.TRUE.equals(paid)) {
                logger.warn("autoCancel 终止：查单显示已支付，orderNo={}", storeOrder.getOrderId());
                return false;
            }
        } catch (Exception e) {
            logger.warn("autoCancel 关单前查单失败，暂不取消，orderNo={}, msg={}", storeOrder.getOrderId(), e.getMessage());
            return false;
        }
        WeChatPayChannelConfig payConfig = weChatPayConfigSupport.resolveByChannel(storeOrder.getIsChannel(), storeOrder.getStoreId());
        try {
            wechatNewService.payOrderClose(storeOrder.getOutTradeNo(), payConfig);
        } catch (Exception e) {
            logger.warn("autoCancel 微信关单失败，暂不取消，orderNo={}, msg={}", storeOrder.getOrderId(), e.getMessage());
            return false;
        }
        return true;
    }
