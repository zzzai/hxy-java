...
public class StoreOrderController {
    ...
    /**
     * 总部退款工单池分页列表（refund_ticket）
     */
    @PreAuthorize("hasAuthority('admin:order:list')")
    @ApiOperation(value = "总部退款工单池分页列表")
    @RequestMapping(value = "/refund/ticket/list", method = RequestMethod.GET)
    public CommonResult<CommonPage<StoreOrderDetailResponse>> getRefundTicketList(
            @Validated StoreOrderSearchRequest request,
            @Validated PageParamRequest pageParamRequest) {
        request.setRefundTicketOnly(true);
        return CommonResult.success(storeOrderService.getAdminList(request, pageParamRequest));
    }
