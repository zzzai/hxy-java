...
<template>
  <s-layout title="申请售后">
    ...
    <view class="compat-tip ss-m-x-20 ss-m-b-20" v-if="state.compatTip">{{ state.compatTip }}</view>
...
<script setup>
  ...
  async function submit() {
    ...
    const result = await AfterSaleApi.createAfterSale(data);
    if (result?.compat?.degraded && result.compat.hint) {
      state.compatTip = result.compat.hint;
    }
    if (result?.code === 0) {
      uni.showToast({
        title: '申请成功',
    ...
    if (result?.compat?.degraded) {
      uni.showModal({
        title: '服务降级提示',
        content: `${result.compat.hint}\n可先联系客服完成售后处理。`,
        cancelText: '稍后再试',
        confirmText: '联系客服',
        success: (res) => {
          if (res.confirm) {
  ...
  onLoad(async (options) => {
    ...
    // 读取订单信息
    const orderDetailResult = await OrderApi.getOrderDetail(state.orderId);
    if (orderDetailResult?.compat?.degraded && orderDetailResult.compat.hint) {
      state.compatTip = orderDetailResult.compat.hint;
    }
    const { code, data } = orderDetailResult || {};
    if (code !== 0 || !data) {
      return;
    }
    state.order = data;
