...
<template>
  <s-layout title="售后列表">
    ...
    <view class="compat-tip ss-m-x-20 ss-m-t-12" v-if="state.compatTip">{{ state.compatTip }}</view>
...
<script setup>
  ...
  async function getOrderList() {
    ...
    const result = await AfterSaleApi.getAfterSalePage({
    ...
    list.forEach((order) => handleAfterSaleButtons(order));
