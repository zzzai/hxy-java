...
<template>
  <s-layout :bgStyle="{ color: '#FFF' }" title="支付结果">
    <view class="pay-result-box ss-flex-col ss-row-center ss-col-center">
      ...
      <view class="compat-tip ss-m-b-20" v-if="state.compatTip">{{ state.compatTip }}</view>
...
<script setup>
  ...
  function applyCompatNotice(result) {
  ...
  async function getOrderInfo(id) {
    state.counter++;
    // 1. 加载订单信息
    const payOrderResult = await PayOrderApi.getOrder(id, true);
    applyCompatNotice(payOrderResult);
    const { data, code } = payOrderResult || {};
    if (code === 0) {
      ...
      if (state.orderInfo.status !== 0) {
        ...
        if (state.orderType === 'goods') {
          const tradeOrderResult = await OrderApi.getOrderDetail(
            state.orderInfo.merchantOrderId,
            true,
          );
          applyCompatNotice(tradeOrderResult);
          const { data, code } = tradeOrderResult || {};
          if (code === 0) {
