...
import { attachCompat, degradedSuccess, isSuccessResult } from '@/sheep/api/compat';
...
const OrderApi = {
  ...
  getOrderDetail: async (id, sync) => {
    ...
    // 兼容旧后端：历史上可能仅支持 /trade/order/get
    ...
    if (isSuccessResult(fallbackResult)) {
      return attachCompat(fallbackResult, {
        endpoint: '/trade/order/get-detail',
        fallbackEndpoint: '/trade/order/get',
        reason: 'ORDER_DETAIL_LEGACY_FALLBACK',
        hint: '后端未完全升级，已回退旧订单详情接口，不影响主流程。',
      });
    }
    ...
    return degradedSuccess({
      endpoint: '/trade/order/get-detail',
      data: null,
      reason: 'ORDER_DETAIL_UNSUPPORTED',
      msg: '当前后端暂不支持订单详情接口，已降级为基础展示，请稍后重试或联系客服。',
    });
  },
  // 订单列表
  getOrderPage: async (params) => {
    ...
    return degradedSuccess({
      endpoint: '/trade/order/page',
      data: {
        list: [],
        total: 0,
      },
      reason: 'ORDER_PAGE_UNAVAILABLE',
      msg: '订单列表接口暂不可用，已降级为空列表展示，不影响下单与支付。',
    });
  },
  // 确认收货
  receiveOrder: (id) => {
