...
const adaptTenant = async () => {
  ...
  try {
    ...
    // 兜底：无法解析租户时，回退到默认租户，避免使用历史缓存的错误 tenant-id。
    if (!newTenantId && defaultTenantId) {
      newTenantId = Number(defaultTenantId);
    }
...
const adaptTemplate = async (appTemplate, templateId) => {
  let { data: diyTemplate } = templateId
    ? // 查询指定模板，一般是预览时使用
      await DiyApi.getDiyTemplate(templateId)
    : await DiyApi.getUsedDiyTemplate();

  // 兜底：本地缓存了错误 tenant-id 时，会出现 code=0 但 data=null，导致误判“未找到模板”。
  // 对小程序联调场景，回退到默认租户后重试一次，避免首次启动即白屏。
  if (!templateId && !diyTemplate) {
    const currentTenantId = getTenantId();
    const fallbackTenantId = Number(defaultTenantId);
    if (fallbackTenantId && Number(currentTenantId) !== fallbackTenantId) {
      uni.setStorageSync('tenant-id', fallbackTenantId);
      const retryResp = await DiyApi.getUsedDiyTemplate();
      diyTemplate = retryResp?.data;
      if (diyTemplate) {
        console.warn('diy template loaded by fallback tenant-id:', fallbackTenantId);
      }
    }
  }
  ...
  appTemplate.home = normalizeDiyPageProperty(diyTemplate?.home);
  appTemplate.user = normalizeDiyPageProperty(diyTemplate?.user);
};

/** 规整装修页数据，避免后端返回 {} 导致小程序白屏 */
const normalizeDiyPageProperty = (pageProperty) => {
  const normalized = pageProperty && typeof pageProperty === 'object' ? pageProperty : {};
  return {
    page: normalized.page && typeof normalized.page === 'object' ? normalized.page : {},
    navigationBar:
      normalized.navigationBar && typeof normalized.navigationBar === 'object'
        ? normalized.navigationBar
        : {},
